...
def render_user_journey_steps():
    """Render visual user journey progress"""
    ...
    with col3:
        profile_complete = st.session_state.get('user_journey', {}).get('profile_completed', False)
        step_class = "completed" if profile_complete else ("active" if st.session_state.current_step == 'profile' else "")
        
        st.markdown(f'''
        <div class="journey-step {step_class}">
            <h3>3. 👤 Profile Setup</h3>
            <p>Build career profile</p>
        </div>
        ''', unsafe_allow_html=True)
    
    # Step 4: Resume & Analysis
    with col4:
        resume_uploaded = st.session_state.get('user_journey', {}).get('resume_uploaded', False)
        step_class = "completed" if resume_uploaded else ("active" if st.session_state.current_step == 'resume' else "")
        
        st.markdown(f'''
        <div class="journey-step {step_class}">
            <h3>4. 📄 AI Analysis</h3>
            <p>Upload & get insights</p>
        </div>
        ''', unsafe_allow_html=True)
...
def main():
    """
    Main application entry point - Routes to 00_Home.py
    00_Home.py contains: Welcome page, Logo, USPs, Payment Plans, Login/Registration
    """
    ...
    try:
        ...
        # Log main app access
        if ERROR_HANDLER_AVAILABLE:
            log_user_action("main_app_entry", {
                'timestamp': datetime.now().isoformat(),
                'routing_to': 'pages/00_Home.py'
            })
        
        # Route directly to 00_Home.py for complete welcome experience
        st.switch_page("pages/00_Home.py")
    
    except Exception as e:
        # Error handling with fallback
        error_message = f"Main application routing error: {str(e)}"
        
        if ERROR_HANDLER_AVAILABLE:
            log_error("main_app_routing_error", error_message, {
                'traceback': traceback.format_exc()
            })
        
        st.error(f"⚠️ Routing Error: {error_message}")
        
        # Show fallback UI with direct access
        st.markdown("""
        # 🚀 IntelliCV-AI - Intelligent Career Platform
        
        **Routing to Welcome Page...**
        
        If automatic routing fails, please access:
        - [🏠 Welcome & Login](pages/00_Home.py)
