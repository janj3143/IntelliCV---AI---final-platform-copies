...
class AIContentGenerator:
    ...
    def optimize_content_for_ats(self, content: str, target_role: str, keywords: Optional[List[str]] = None) -> str:
        ...
        # Load keywords from AI data instead of hardcoded dictionary
        role_keywords = {}
        
        if AI_LOADER_AVAILABLE and ai_loader:
            try:
                # Load real job titles with their skills
                job_titles_data = ai_loader.load_real_job_titles()
                if job_titles_data and 'job_titles' in job_titles_data:
                    for job in job_titles_data['job_titles']:
                        if isinstance(job, dict):
                            title = job.get('title', '').lower()
                            skills = job.get('required_skills', job.get('skills', []))
                            if title and skills:
                                role_keywords[title] = skills[:8] if isinstance(skills, list) else [skills]
            except Exception:
                pass
        ...
        if not role_keywords:
            role_keywords = {
                "software engineer": ["Problem Solving", "Technical Design", "Team Collaboration", "Testing"],
                "data analyst": ["Data Analysis", "Visualization", "Communication", "Statistics"],
                "project manager": ["Planning", "Leadership", "Communication", "Risk Management"]
            }
