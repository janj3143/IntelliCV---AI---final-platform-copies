...
class WebCompanyIntelligence:
    ...
    def load_sample_data(self):
        ...
        # Load company database from AI data instead of hardcoded
        ...
        if AI_LOADER_AVAILABLE and ai_loader:
            try:
                ...
                if companies_data and isinstance(companies_data, dict):
                    # Convert AI data to expected format
                    for company_name, company_info in list(companies_data.items())[:20]:
                        company_key = company_name.lower().replace(' ', '_')
                        self.company_database[company_key] = {
                            "company_name": company_name,
                            "industry": company_info.get('industry', 'Technology') if isinstance(company_info, dict) else 'Technology',
                            "headquarters": company_info.get('location', 'N/A') if isinstance(company_info, dict) else 'N/A',
                            "employees": company_info.get('size', 'N/A') if isinstance(company_info, dict) else 'N/A',
                            "confidence_score": company_info.get('count', 1) * 10 if isinstance(company_info, dict) else 50,
                            "technologies": [],  # Load from Exa API when needed
                            "culture_keywords": company_info.get('contexts', []) if isinstance(company_info, dict) else []
                        }
            except Exception:
                pass
