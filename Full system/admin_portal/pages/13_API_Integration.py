    ...
    def get_api_keys(self) -> List[Dict[str, Any]]:
        ...
        return [
            # AI/LLM Services
            {
                ...
                "Required": "Critical",
                "Purpose": "Resume analysis, chatbot, career intelligence",
            ...
            },
            {
                ...
                "Validity_Period": "No expiration (billing-based)",
                "Required": "Optional",
                "Purpose": "Code fix suggestions, enhanced search",
            ...
            # Communication Services
            {
                ...
                "Required": "High",
                "Purpose": "Email notifications, user communications",
            ...
            # Search & Data APIs
            {
                ...
                "Validity_Period": "No expiration (billing-based)",
                "Required": "Optional",
                "Purpose": "Enhanced semantic search, web data",
            ...
            },
            {
                ...
                "Validity_Period": "No expiration",
                "Required": "Optional",
                "Purpose": "Google search results, job listings",
            ...
            # Development & Testing
            {
                ...
                "Required": "Optional",
                "Purpose": "API testing, collection management",
...
def render_api_key_card(api: Dict[str, Any]) -> None:
    ...
    with st.expander(f"{status_color} {api['Service']} - {api['Provider']}", expanded=False):
        ...
        key_input = st.text_input(
            ...
            key=f"key_{api['Service'].replace(' ', '_').replace('/', '_')}",
        ...
        with col_btn1:
            if st.button("💾 Save", key=f"save_{api['Service'].replace(' ', '_').replace('/', '_')}", help="Save API Key"):
                if key_input:
        ...
        with col_btn2:
            if st.button("🧪 Test", key=f"test_{api['Service'].replace(' ', '_').replace('/', '_')}", help="Test API Connection"):
                if key_input:
        ...
        with col_btn3:
            if st.button("📊 Stats", key=f"stats_{api['Service'].replace(' ', '_').replace('/', '_')}", help="View Usage Stats"):
                st.info(f"📊 Usage statistics for {api['Service']} would be shown here")
        
        with col_btn4:
            if st.button("📋 Copy", key=f"copy_{api['Service'].replace(' ', '_').replace('/', '_')}", help="Copy Documentation URL"):
...
def render():
    ...
    def _render_api_key_card(self, api: Dict[str, Any]) -> None:
        """Render an individual API key card with configuration options."""
        status_color = "🟢" if "Active" in api['Status'] else "🟡" if "Testing" in api['Status'] else "⚠️"
        ...
        with st.expander(f"{status_color} {api['Service']} - {api['Provider']}", expanded=False):
            ...
            key_input = st.text_input(
                ...
                type="password",
                placeholder="Paste your API key here...",
                key=f"key_{api['Service'].replace(' ', '_')}",
                help=f"Enter the API key for {api['Service']}. This will be securely stored."
            )
            ...
            with col_btn1:
                if st.button("💾 Save", key=f"save_{api['Service']}", help="Save API Key"):
                    if key_input:
                        st.success(f"✅ API key saved for {api['Service']}")
                        st.info("💡 Remember to restart the application for changes to take effect")
                    else:
            ...
            with col_btn2:
                if st.button("🧪 Test", key=f"test_{api['Service']}", help="Test API Connection"):
                    if key_input:
                        with st.spinner("Testing connection..."):
                            time.sleep(1)
                            st.success("✅ API connection successful!")
            ...
            with col_btn3:
                if st.button("📊 Stats", key=f"stats_{api['Service']}", help="View Usage Stats"):
                    st.info(f"📊 Usage statistics for {api['Service']} would be shown here")
            
            with col_btn4:
                if st.button("� Copy", key=f"copy_{api['Service']}", help="Copy Documentation URL"):
                    st.code(api['Documentation'])
                    st.success("📋 Documentation URL ready to copy")
