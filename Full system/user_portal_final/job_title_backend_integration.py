...
class JobTitleBackend:
    ...
    def _extract_job_titles_from_real_data(self) -> List[str]:
        ...
        try:
            ...
            # Look for real data in ai_data_final directories
            ai_data_paths = [
                Path("ai_data_final"),
                Path("../ai_data_final"),
                Path("../../ai_data_final"),
            ...
            for ai_path in ai_data_paths:
                if ai_path.exists():
                    logger.info(f"Found AI data directory: {ai_path}")
    ...
    def _fallback_career_pathways(self, current_role: str) -> List[Dict]:
        """Fallback career pathways when AI engine is unavailable."""
        return [
            {
                'pathway_type': 'Standard Progression',
                'roles': ['Senior Role', 'Lead Role', 'Management Role'],
                'timeline': '2-5 years',
                'skills_needed': ['Leadership', 'Communication', 'Technical Skills'],
                'confidence': 0.7,
    ...
    def _fallback_market_intelligence(self, job_title: str) -> Dict:
        """Fallback market intelligence when AI is unavailable."""
        return {
            'job_title': job_title,
            'market_analysis': {
                'demand_forecast': 'Medium',
                'salary_trend': 'Stable',
                'skills_in_demand': ['Technical Skills', 'Communication'],
    ...
    def _fallback_title_relationships(self, job_title: str) -> Dict:
        """Fallback title relationships when AI is unavailable."""
        return {
            'primary_title': job_title,
            'relationships': {
                'similar_titles': ['Similar Role', 'Related Position'],
                'prerequisite_roles': ['Junior Role'],
                'advancement_roles': ['Senior Role'],
                'lateral_moves': ['Cross-functional Role']
            },
            'source': 'fallback_mode'
        }
    ...
    def _fallback_word_cloud(self, **kwargs) -> Dict:
        """Fallback word cloud when AI is unavailable."""
        return {
            'word_cloud_data': {
                'Technical': 0.8,
                'Skills': 0.7,
    ...
    def generate_word_cloud(self, job_titles: List[str], source_page: str = "user_portal") -> Optional[str]:
        ...
        if not self.service_available and not self.ai_services_available:
            ...
            try:
                from ai_data_loader import get_real_job_titles, get_ai_insights
    ...
    def analyze_job_title(self, title: str, context: Optional[str] = None) -> Optional[Dict]:
        ...
        if not self.service_available and not self.ai_services_available:
            ...
            try:
                from ai_data_loader import get_ai_insights, get_real_skills
    ...
    def get_career_pathways(self, current_role: Optional[str] = None) -> Optional[List[Dict]]:
        ...
        if not self.service_available:
            ...
            try:
                from ai_data_loader import get_ai_insights, get_real_job_titles
    ...
    def get_market_intelligence(self, job_category: Optional[str] = None) -> Optional[Dict]:
        ...
        if not self.service_available:
            ...
            try:
                from ai_data_loader import get_ai_insights, get_real_companies
