...
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "admin_portal"))
...
class DashboardEngine:
    ...
    def _load_user_data(self) -> Dict[str, Any]:
        """Load user profile and progress data from AI engine and session state"""
        
        # Try to get real user data from session state or AI engine
        if AI_LOADER_AVAILABLE and ai_loader:
            try:
                # Get real skills from AI data
                real_skills_data = ai_loader.load_real_skills_data()
                skills_list = list(real_skills_data.keys())[:6] if real_skills_data else []
                
                # Get real job titles
                job_titles_data = ai_loader.load_real_job_titles()
                if job_titles_data and 'job_titles' in job_titles_data:
                    sample_title = job_titles_data['job_titles'][0]['title'] if job_titles_data['job_titles'] else "Professional"
                else:
        ...
        # Check session state for user data
        user_name = st.session_state.get('user_name', 'Career Professional')
        user_email = st.session_state.get('user_email', 'user@example.com')
        
        # Use real data when available, fallback to minimal defaults
        return {
            "profile": {
                "name": user_name,
                "title": st.session_state.get('user_title', sample_title),
                "experience_years": st.session_state.get('experience_years', 5),
                "target_role": st.session_state.get('target_role', 'Next Career Level'),
            ...
            "career_position": {
                "skills_score": st.session_state.get('skills_score', 75),
                "market_demand": st.session_state.get('market_demand', 80),
                "quadrant": st.session_state.get('quadrant', 'Rising Star'),
                "peer_percentile": st.session_state.get('peer_percentile', 70)
            },
            "engagement": {
                "level": st.session_state.get('engagement_level', 'Active User'),
